...
StatusRemocao fundir_com_irmao_esquerdo(Arv23 **ponteiro_filho_no_pai, Arv23 *pai, Arv23 *irmao_esq) {
    ...
    // Salva filhos do no em underflow - CORRIGIDO: pega os filhos corretos
    ...
    // Realiza fusao no irmao esquerdo - CORRIGIDO
    irmao_esq->info2 = chave_pai_desce;
    irmao_esq->nInfo = 2;
    // CORREÇÃO PRINCIPAL: O filho central do irmão esquerdo vira o filho direito
    // e os filhos do nó em underflow são distribuídos corretamente
    irmao_esq->dir = filho_u_esq; // O primeiro filho do nó em underflow

    // Atualiza ponteiro no pai para apontar para o irmão esquerdo
    *ponteiro_filho_no_pai = irmao_esq;
...
StatusRemocao fundir_com_irmao_direito(Arv23 **ponteiro_filho_no_pai, Arv23 *pai, Arv23 *irmao_dir) {
    ...
    // Realiza fusao no irmao direito - CORRIGIDA
    ...
    // CORREÇÃO: Reorganiza os filhos corretamente
    irmao_dir->dir = irmao_d_cen;  // Filho central original vira direito
    irmao_dir->cen = irmao_d_esq;  // Filho esquerdo original vira central
    irmao_dir->esq = filho_u_esq;  // Filho do nó em underflow vira esquerdo

    // Atualiza ponteiro no pai
    *ponteiro_filho_no_pai = irmao_dir;
...
int main()
{
    ...
    do
    {
        ...
        case 2:
            ...
            {
                printf("--- Erro ao remover valor %d ---\n", valor);
            }
            break;
