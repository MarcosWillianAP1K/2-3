...
StatusRemocao fundir_com_irmao_esquerdo(Arv23 **ponteiro_filho_no_pai, Arv23 *pai, Arv23 *irmao_esq, int pos_filho) {
    ...
    } else { // Pai era 3-no, ficara 2-no
        if (pos_filho == 1) { // Filho era o central (pai->cen)
            chave_pai_desce = pai->info1;
            pai->info1 = pai->info2;
            printf("Pai [3-no], filho=cen. Chave descendo: %d. Pai vira [%d]\n", chave_pai_desce, pai->info1);
        } else { // pos_filho == 2 (Filho era o direito, pai->dir)
            chave_pai_desce = pai->info2;
...
StatusRemocao fundir_com_irmao_direito(Arv23 **ponteiro_filho_no_pai, Arv23 *pai, Arv23 *irmao_dir, int pos_filho) {
    ...
    if (pai->nInfo == 1) {
        ...
        if (pos_filho == 0) { // Filho era o esquerdo (pai->esq)
            chave_pai_desce = pai->info1;
            pai->info1 = pai->info2;
            printf("Pai [3-no], filho=esq. Chave descendo: %d. Pai vira [%d]\n", chave_pai_desce, pai->info1);
        } else { // pos_filho == 1 (Filho era o central, pai->cen)
            chave_pai_desce = pai->info2;
             printf("Pai [3-no], filho=cen. Chave descendo: %d. Pai vira [%d]\n", chave_pai_desce, pai->info1);
        }
        pai->info2 = 0;
    ...
    if (pos_filho == 0) { // Filho era pai->esq. Irmao dir era pai->cen.
    ...
    } else { // pos_filho == 1. Filho era pai->cen. Irmao dir era pai->dir.
...
//================ REMOCAO (Recursiva) ==================
StatusRemocao remover_23_recursivo(Arv23 **ponteiro_no_atual, int valor, Arv23 *pai_do_no_atual) {
    StatusRemocao status_final = OK;
        ...
        if (valor_encontrado_neste_no) {
            ...
            if (eh_folha(no_atual)) {
                ...
                if (sucessor_node == NULL) {
                    ...
                    StatusRemocao status_remocao_sucessor = remover_23_recursivo(ponteiro_subarvore_sucessor, valor_sucessor, no_atual);
            ...
            StatusRemocao status_recursao = remover_23_recursivo(proximo_ponteiro_recursao, valor, no_atual);
...
int remover_23(Arv23 **raiz, int valor) {
    ...
    if (raiz == NULL || *raiz == NULL) {
        ...
        StatusRemocao status_geral = remover_23_recursivo(raiz, valor, NULL);
        ...
        if (status_geral == OK && (*raiz == NULL || (*raiz)->nInfo > 0)) {
        ...
        if (maiorNo != NULL)
        {
            ...
            if (nova_raiz != NULL) {
                *raiz = nova_raiz;
                ...
                if (maiorNo) free(maiorNo); // Tenta liberar
            }
        }
    }
    return sucesso;
...
void libera_arvore(Arv23 *raiz) {
    if (raiz != NULL) {
        // Verifica nInfo antes de acessar filhos para evitar crash se a arvore estiver corrompida
        if (raiz->nInfo > 0) {
        ...
        // printf("Liberando no [%d%s%d] (%p)\n", raiz->info1, raiz->nInfo==2 ? ", " : " ", raiz->nInfo==2 ? raiz->info2 : -1, (void*)raiz);
        free(raiz);
    }
}
